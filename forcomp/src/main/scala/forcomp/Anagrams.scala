...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    for ((c, cs) <- w groupBy (_.toLower))
      yield (c, cs.length)
  }.toList.sorted

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    for ((c, os) <- s flatMap wordOccurrences groupBy (_._1))
      yield (c, os.map(_._2).sum)
  }.toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences match {
      ...
      case head :: tail => (for {
        i <- 0 to head._2
        others <- combinations(tail)
      } yield
        if (i > 0) (head._1, i) :: others
        else others).toList
    }
