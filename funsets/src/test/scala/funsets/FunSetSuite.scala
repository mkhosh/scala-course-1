...
class FunSetSuite extends FunSuite {
  ...
  test("intersect") {
    new TestSets {
      ...
      val i2 = intersect(s1, u)
      assert(contains(i2, 1), "Intersect 1")
      assert(!contains(i2, 2), "Intersect 1")
  ...
  test("forall") {
    new TestSets {
      val f = union(s3, union(s1, s2))
      assert(forall(f, _ > 0), "forall 1")
      assert(!forall(f, _ < 0), "forall 1")
      assert(!forall(f, _ < 2), "forall 1")
    }
  }
  ...
  test("exists") {
    new TestSets {
      val f = union(s3, union(s1, s2))
      assert(exists(f, _ > 0), "exists 1")
      assert(!exists(f, _ < 0), "exists 1")
      assert(exists(f, _ < 2), "exists 1")
    }
  }
  ...
  test("map") {
    new TestSets {
      ...
      assert(contains(f,4), "map 2")
      assert(!contains(f,2), "map 3")
    }
  }
